...
class mesh_simplify(a_3d_model):
    ...
    def generate_valid_pairs(self):
        ...
        for i in range(0, self.number_of_points):
            ...
            current_point_to_others_dist = (
                np.sum((self.points-current_point)**2, axis=1))**0.5
            # np.where返回的是一个元组,[0]包含满足的数组，包含找到的顶点索引==> 元组内数组均+1到valid_pairs_location中
            valid_pairs_location = np.where(
                current_point_to_others_dist <= self.t)[0]+1
        ...
        if self.dist_pairs.size > 0:
            # 边与有效点对进行拼接
            self.valid_pairs = np.concatenate(
                [self.edges, self.dist_pairs], axis=0)
    ...
    def calculate_optimal_contraction_pairs_and_cost(self):
        ...
        for i in range(0, number_of_valid_pairs):
            ...
            if np.linalg.det(Q_new) > 0:
                ...
                # 获取最小的成本及位置
                current_cost = np.min(
                    np.array([delta_v_1, delta_v_2, delta_v_mid]))
                min_delta_loc = np.argmin(
                    np.array([delta_v_1, delta_v_2, delta_v_mid]))
                current_v_opt = np.concatenate([v_1, v_2, v_mid], axis=1)[
                    :, min_delta_loc].reshape(4)
    ...
    def iteratively_remove_least_cost_valid_pairs(self):
        ...
        while (self.number_of_points-self.new_point_count) >= self.ratio*(self.number_of_points):
            ...
            # v_2_in_faces_loc 元组包含两个，若是矩阵[0]指的是行，[1]指的是列
            ...
            if self.new_point_count % 100 == 0:
                print('Simplification: '+str(100*(self.number_of_points -
                      self.new_point_count)/(self.number_of_points))+'%')
                print('Remaining: '+str(self.number_of_points -
                      self.new_point_count)+' points')
                print('\n')

            self.new_point_count = self.new_point_count+1

        print('Simplification: '+str(100*(self.number_of_points -
              self.new_point_count)/(self.number_of_points+self.new_point_count))+'%')
        print('Remaining: '+str(self.number_of_points -
              self.new_point_count)+' points')
        print('End\n')
    ...
    def update_plane_equation_parameters(self, need_updating_loc):
        ...
        for i in need_updating_loc:
            if self.status_faces[i] == -1:
                self.plane_equ_para[i, :] = np.array(
                    [0, 0, 0, 0]).reshape(1, 4)
            else:
                # 将面中的点取出
    ...
    def update_valid_pairs_v_optimal_and_cost(self, target_loc):
        ...
        v_1_loc_in_valid_pairs = np.where(
    ...
    def output(self, output_filepath):
        with open(output_filepath, 'w') as file_obj:
            file_obj.write('# '+str(self.number_of_points) +
                           ' vertices, '+str(self.number_of_faces)+' faces\n')
